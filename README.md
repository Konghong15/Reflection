...
루트 객체를 기준으로 참조 그래프를 순회하여, 사용 중인 객체만 추적하는 마크 앤 스윕(Mark and Sweep) 기반 GC입니다.
수거 대상이 된 객체를 안전하게 식별하고 해제할 수 있음을 확인합니다.
현재는 멀티스레드에 대한 이해와 최적화가 부족하여, 멀티스레딩 사용 시 오히려 성능이 저하되는 문제가 있습니다.
---
...
현재는 네트워크 연동 기능은 구현되어 있지 않아, 원격 호출 처리는 미지원 상태입니다.
